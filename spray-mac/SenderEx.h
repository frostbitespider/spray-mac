...
public:
    ...
    SA* dstAddr;
    socklen_t dstlen;
    void setHeader(MessageHeader mh){
        sendhdr.mh=mh;
    }


}
// ssize_t send_recv(SenderEx* s){
//     int pthread_t tid;
//     pthread_create(tid,send_recv,(void*)s);
// }
// ssize_t send_recv(void* s){
//     s=(SenderEx*)s;
//     return send_and_recv(s->fd,s->outbuff,s->outbytes,s->inbuff,s->inbytes,s->dstAddr,s->dstlen);
// }
